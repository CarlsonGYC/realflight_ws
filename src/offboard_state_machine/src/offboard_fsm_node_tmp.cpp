...
void OffboardFSM::timer_cb()
{
  ...
  case FsmState::TAKEOFF: {
    double elapsed   = offb_counter_ * timer_period_s_;
    double climb_rate= takeoff_alt_ / takeoff_time_s_;
    double alt_sp    = std::min(takeoff_alt_, climb_rate * elapsed);
    
    TrajectorySetpoint sp{};
    // Position setpoints (required)
    sp.position[0] = takeoff_pos_x_;
    sp.position[1] = takeoff_pos_y_;
    sp.position[2] = -alt_sp;  // NED: negative is up
    
    // Velocity setpoints (set to NaN to let PX4 handle, or 0.0 for no velocity)
    sp.velocity[0] = std::nanf("");  // or 0.0f
    sp.velocity[1] = std::nanf("");  // or 0.0f
    sp.velocity[2] = std::nanf("");  // or 0.0f
    
    // Acceleration setpoints (set to NaN)
    sp.acceleration[0] = std::nanf("");
    sp.acceleration[1] = std::nanf("");
    sp.acceleration[2] = std::nanf("");
    
    // Yaw setpoint (set to NaN or specific value)
    sp.yaw = std::nanf("");  // Let PX4 maintain current yaw
    ...
    bool   alt_ok     = actual_alt >= takeoff_alt_ - alt_tol_;
    bool   vel_ok     = std::abs(current_z_ - last_z_) < 0.02;
    ...
    if (hold_done) {
      if (has_goto_target()) {
        current_state_ = FsmState::GOTO;
        RCLCPP_INFO(get_logger(), "Transitioning to GOTO");
      } else {
        // Initialize hover position when no goto target
        hover_x_ = takeoff_pos_x_;
        hover_y_ = takeoff_pos_y_;
        hover_z_ = -takeoff_alt_;
        current_state_ = FsmState::HOVER;
        RCLCPP_INFO(get_logger(), "Transitioning to HOVER at takeoff position");
      }
      state_start_time_ = now();
      offb_counter_ = 0;
    }
  ...
  case FsmState::GOTO: {
    ...
    // Velocity setpoints (NaN = let controller decide)
    ...
    if (err < goto_tol_) {
        RCLCPP_INFO(get_logger(), "Reached GOTO target (error: %.3f m)", err);
        hover_x_ = goto_x_;
        hover_y_ = goto_y_;
        hover_z_ = goto_z_;
        current_state_ = FsmState::HOVER;
...
void OffboardFSM::publish_offboard_mode()
{
  ...
  OffboardControlMode m{};
  m.position     = true;   // Using position control
  m.velocity     = false;  // Not using velocity control
  m.acceleration = false;  // Not using acceleration control
  m.attitude     = false;  // Not using attitude control  
  m.body_rate    = false;  // Not using body rate control
  m.timestamp    = now().nanoseconds() / 1000ULL;
  pub_offb_mode_->publish(m);
}
